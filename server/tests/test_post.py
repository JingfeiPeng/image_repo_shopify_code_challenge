...
@pytest.fixture
def image():
    with open(join(os.path.dirname(os.path.realpath(__file__)), TESTING_IMAGE), "rb") as f:
        yield f

@pytest.fixture
def cat2():
    with open(join(os.path.dirname(os.path.realpath(__file__)), 'cat_2.jpg'), "rb") as f:
        yield f

@pytest.fixture
def cat3():
    with open(join(os.path.dirname(os.path.realpath(__file__)), 'cat_3.jpg'), "rb") as f:
        yield f
...
def test_private_post(client, image):
    ...
    rv = client.get('/images', query_string={'user':'another_user',})
    list_of_images = json.loads(rv.data)
    assert rv.status_code == 200
    assert len(list_of_images) == 0
